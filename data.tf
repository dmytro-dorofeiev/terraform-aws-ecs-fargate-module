...
data "aws_iam_policy_document" "tasks_execution_ssm" {
  statement {
    effect = "Allow"

    resources = [
      "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${var.ssm_allowed_parameters}",
      "${var.kms_key_arn}"
    ]
