...
resource "aws_iam_role_policy" "tasks_execution_ssm" {
  count = var.ssm_allowed_parameters != "" ? 1 : 0

  name   = "${var.name_prefix}-read-secrets-ssm"
  role   = aws_iam_role.execution.id
  policy = data.aws_iam_policy_document.tasks_execution_ssm.json
}
...
resource "aws_security_group_rule" "egress_service" {
  description       = "Allow egress traffic anywhere"
...
resource "aws_ecs_service" "service" {
  name = var.service_name != "" ? var.service_name : var.name_prefix
